...
correlation_id="$(printf '%s\n' "$output" | grep -E '^correlation_id=' | sed 's/correlation_id=//')"
http_status="$(printf '%s\n' "$output" | grep -E '^http_status=' | sed 's/http_status=//')"
...
docker compose logs orders-service --since=3m | grep -E 'operation=kafka_event_published' | grep -E "$correlation_id" >/dev/null || {
  echo "[FAIL] missing orders-service kafka publish evidence for correlation_id=${correlation_id}" >&2
  exit 1
}

docker compose logs inventory-service --since=3m | grep -E 'operation=kafka_processing_failed' | grep -E "$correlation_id" >/dev/null || {
  echo "[FAIL] missing inventory-service kafka processing failure for correlation_id=${correlation_id}" >&2
  exit 1
}

docker compose logs inventory-service --since=3m | grep -E 'PoisonMessageException|Simulated poison message' | grep -E "$correlation_id" >/dev/null || {
  echo "[FAIL] missing poison-message exception evidence for correlation_id=${correlation_id}" >&2
  exit 1
}

if docker compose logs inventory-service --since=3m | grep -E 'operation=kafka_dlq_published' | grep -E "$correlation_id" >/dev/null; then
  echo "[OK] DLQ publish evidence found in inventory-service logs"
else
  if docker compose exec -T redpanda rpk topic consume order-created-dlq -n 20 -f '%h\\n%v\\n' 2>/dev/null | grep -E "$correlation_id" >/dev/null; then
