...
echo "- Loki query: {service_name=~\"api-gateway|orders-service|payments-service\"} |= \"${RESP_CORRELATION_ID}\" |= \"500\""
echo "- Tempo: inspect orders-service -> payments-service span status for trace ID '${TRACE_ID:-<copy-from-logs>}'"
echo "- Note: enable payments failure mode first (e.g., failure-simulation.payments.failure-rate > 0)."
