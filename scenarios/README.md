...
| S006 — Pub/Sub publish failure | Implemented | `docker-compose.s006.yml` | `async` | `./scripts/trigger-s006-pubsub-publish-failure.sh` | `./scripts/verify-s006-pubsub-publish-failure.sh` | HTTP 503 + `NOTIFICATION_PUBLISH_FAILED` with publish-failure logs | Requires S006 override to force failure deterministically |
